...
- No PR touches `CHANGELOG.md`, `pyproject.toml`, or `server.json`: the release
  ...
  body, ready to paste.
- Fork-specific bits (build stamps, my auto-update launcher) are stripped. The
...
I hit #106 before your 0.4.3 and shipped my own lock; you've since solved it
with `IndexLock`, so I'm not proposing a replacement. I did find one bug in it
...
| A8 · harden `IndexLock` | Against your 0.4.3 code, not my fork. `try_acquire()` treats any `flock` `OSError` as contention, so a non-locking filesystem wedges the index permanently and a read-only home raises uncaught. Now distinguishes real contention from an unlockable filesystem (degrade and warn). See #106 above. |
...
| Unit | Adds |
|---|---|
| E1 · one-click Claude Desktop install (`.mcpb`) | A double-click desktop-extension bundle (`manifest_version 0.3`): the user installs by opening the file, with no `uv` or `pip` and no JSON editing. A small Node launcher starts the Python server and recovers a missing `uv`, fetching it or explaining clearly rather than failing silently. I'd contribute a generic bundle; my fork's auto-update and build-revision tracking stay behind. |

---

### What I'd like from you

Tell me which units or tracks you want, and whether you'd prefer them per-unit or
...
E1 is a clean extraction of my bundle, both of which I'd prepare fresh). On your
word I open the ones you want as focused PRs, freshly rebased on your current
`main`, with `ruff check`, `ruff format --check`, and `pytest` green. Glad to
split, rescope, or drop any of them.
